...
function loadData() {
    try {
        const sp = localStorage.getItem('pos_products');
        const sh = localStorage.getItem('pos_history');
        
        if(!sp) {
            // Data Dummy Awal
            products = [
                {id: 1, name: "Kopi Hitam", category: "drink", price: 10000, stock: 100, icon: "fa-mug-hot"},
                {id: 2, name: "Es Jeruk", category: "drink", price: 8000, stock: 50, icon: "fa-glass-water"},
                {id: 3, name: "Nasi Goreng", category: "food", price: 20000, stock: 20, icon: "fa-utensils"},
                {id: 4, name: "Roti Bakar", category: "snack", price: 15000, stock: 30, icon: "fa-bread-slice"},
            ];
        } else {
            products = JSON.parse(sp);
        }
        history = sh ? JSON.parse(sh) : [];
    } catch(e) { products = []; history = []; }
}
